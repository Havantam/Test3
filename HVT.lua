...
Converted["_HVT"].DisplayOrder = 999999999
Converted["_HVT"].IgnoreGuiInset = true
Converted["_HVT"].ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
Converted["_HVT"].ResetOnSpawn = false
Converted["_HVT"].ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Converted["_HVT"].Name = "HVT"
Converted["_HVT"].Parent = game:GetService("CoreGui")

Converted["_Open"].Font = Enum.Font.Gotham
Converted["_Open"].Text = "3 click open menu"
...
Converted["_Open"].Parent = Converted["_HVT"]
...
Converted["_TextLabel1"].Text = "HVT"
...
Converted["_ListButton"].Parent = Converted["_HVT"]
...
Converted["_Notifications"].Parent = Converted["_HVT"]
...
Converted["_TextBoxPlaceholder"].Parent = Converted["_HVT"]
...
Converted["_FloatingButton"].Parent = Converted["_HVT"]
...
Converted["_FloatingButtons"].Parent = Converted["_HVT"]
...
Converted["_AddCustomModule1"].Parent = Converted["_HVT"]
...
Converted["_DroppedGunBGUI"].Parent = Converted["_HVT"]
...
Converted["_MurdererBGUI"].Parent = Converted["_HVT"]
...
do -- Fake Module: StarterGui.HVT.FUNCTIONS
    local script = Instance.new("ModuleScript")
    script.Name = "FUNCTIONS"
    script.Parent = Converted["_HVT"]
		...
		function loader(module)
			...
			for _, item in ipairs(module) do
				...
				elseif item["Type"] == "Button" then
					...
					button.MouseButton1Down:Connect(function()
						...
						task.spawn(function()
							...
							if not _G.HVT.FloatingButtons:FindFirstChild(item["Args"][1]) then
		
		
								local newFloatingButton = _G.HVT.FloatingButton:Clone()
								newFloatingButton.Parent = _G.HVT.FloatingButtons
								...
								_G.HVT.FloatingButtons:FindFirstChild(item["Args"][1]):Destroy()
							end
						end)
					end)
				...
				elseif item["Type"] == "Input" then
					local cloneinput = _G.HVT.TextBoxPlaceholder:Clone()
		...
		function FUNCTIONSmodule.refreshlist()
			...
			for modulei, module in ipairs(_G.Modules) do
				local success, err = pcall(function()
					local listbutton = _G.HVT.ListButton:Clone()
				...
				if not success then
					warn("[HVT] Error while requiring " .. module.Name .. "!")
					warn(err)
				end
			end
		end
...
local function VHDAETB_fake_script() -- Fake Script: StarterGui.HVT.Open.InitOpen
...
local function ZFVSOXJ_fake_script() -- Fake Script: StarterGui.HVT.Open.OnClick
    local script = Instance.new("LocalScript")
    script.Name = "OnClick"
    script.Parent = Converted["_Open"]
    local req = require
	...
	local ts = game:GetService("TweenService")
	repeat task.wait() until _G.HVT
	ts:Create(_G.HVT.Menu, TweenInfo.new(0.7, Enum.EasingStyle.Back, Enum.EasingDirection.Out), 
...
local function FVTD_fake_script() -- Fake Script: StarterGui.HVT.Open.Resizer
    local script = Instance.new("LocalScript")
    script.Name = "Resizer"
    script.Parent = Converted["_Open"]
    local req = require
...
local function PWBW_fake_script() -- Fake Script: StarterGui.HVT.Menu.List.AutoSetup
    local script = Instance.new("LocalScript")
...
local function SLFHRY_fake_script() -- Fake Script: StarterGui.HVT.Menu.Close.LocalScript
    local script = Instance.new("LocalScript")
	...
	script.Parent.MouseButton1Click:Connect(function()
		ts:Create(_G.HVT.Menu, TweenInfo.new(0.4, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), 
			{Position = UDim2.fromScale(0.499, -0.1), Size = UDim2.fromOffset(441, 0)}
		):Play()
	end)
end
local function NKMN_fake_script() -- Fake Script: StarterGui.HVT.Menu.AddCustomModule.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function WUDV_fake_script() -- Fake Script: StarterGui.HVT.Init
    local script = Instance.new("LocalScript")
    script.Name = "Init"
    script.Parent = Converted["_HVT"]
    local req = require
	...
	require(script.Parent.FUNCTIONS).notification("Thanks for using HVT! To use this hub, triple-click/tap the top region of your screen.")

	}
	
	_G.Modules[2] = module
end
local function DVOUD_fake_script() -- Fake Script: StarterGui.HVT.Universal
    local script = Instance.new("LocalScript")
	...
	local fu = require(_G.HVT.FUNCTIONS)
	...
	module[1] = {
		Type = "Text",
		Args = {"Welcome to HVT! The open, free script hub."}
	}
	
	module[2] = {
		Type = "Button",
		Args = {"Move HVT trigger to the bottom", function(Self)
			_G.HVT.Open.UIStroke.Transparency = 0
			_G.HVT.Open.TextTransparency = 0
			ts:Create(_G.HVT.Open, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
				Position = UDim2.fromScale(0.5, 0.903)
			}):Play()
			
			ts:Create(_G.HVT.Open.UIStroke, TweenInfo.new(1), {
				Transparency = 1
			}):Play()
			ts:Create(_G.HVT.Open, TweenInfo.new(1), {
				TextTransparency = 1
			}):Play()
		end}
	}
	...
	module[3] = {
		Type = "Button",
		Args = {"Move HVT trigger to the top", function(Self)
			_G.HVT.Open.UIStroke.Transparency = 0
			_G.HVT.Open.TextTransparency = 0
			ts:Create(_G.HVT.Open, TweenInfo.new(1, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), {
				Position = UDim2.fromScale(0.5, 0.063)
...
local function MKPXTMU_fake_script() -- Fake Script: StarterGui.HVT.FloatingButton.Keybinding
    local script = Instance.new("LocalScript")
...
local function UHFS_fake_script() -- Fake Script: StarterGui.HVT.Murder Mystery 2
    local script = Instance.new("LocalScript")
				...
				if workspace:FindFirstChild("Normal") then
					if workspace:FindFirstChild("Normal"):FindFirstChild("CoinContainer") then
						for _, coin in ipairs(workspace:FindFirstChild("Normal"):FindFirstChild("CoinContainer"):GetChildren()) do
							...
							if (coinPosition - game.Players.LocalPlayer.Character:GetPivot().Position).Magnitude > 30 then return end
			...
			if findSheriff() == game.Players.LocalPlayer and autoShooting then
				...
				repeat
					...
					if not hit or hit.Instance.Parent == murderer.Character then -- Check if nothing collides or if it collides with the murderer
						...
						local predictedPosition = murdererHRP.Position + ((murdererVelocity * Vector3.new(0, 0.5, 0))) * (shootOffset / 15) + murderer.Character.Humanoid.MoveDirection * shootOffset
		...
		Args = {"Kill closest player as murderer (DETECTABLE)", function()
			...
			if NearestPlayer and NearestPlayer.Character then
				...
				if localRootPart and nearestRootPart then
					...
					-- Teleport the local player to the offset position
					localRootPart.CFrame = CFrame.lookAt(localRootPart.CFrame.Position, nearestRootPart.CFrame.Position) * CFrame.new(offsetPosition) * CFrame.Angles(0, nearestRootPart.CFrame.Y, 0)
				end
			else
...
local function VDFN_fake_script() -- Fake Script: StarterGui.HVT.AddCustomModule.Add.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
local function QGLYJ_fake_script() -- Fake Script: StarterGui.HVT.AddCustomModule.Cancel.LocalScript
    local script = Instance.new("LocalScript")
    script.Name = "LocalScript"
...
coroutine.wrap(VHDAETB_fake_script)()
coroutine.wrap(ZFVSOXJ_fake_script)()
coroutine.wrap(FVTD_fake_script)()
coroutine.wrap(PWBW_fake_script)()
coroutine.wrap(SLFHRY_fake_script)()
coroutine.wrap(NKMN_fake_script)()
coroutine.wrap(WUDV_fake_script)()
coroutine.wrap(ZVPMV_fake_script)()
coroutine.wrap(DVOUD_fake_script)()
coroutine.wrap(MKPXTMU_fake_script)()
coroutine.wrap(UHFS_fake_script)()
coroutine.wrap(VDFN_fake_script)()
coroutine.wrap(QGLYJ_fake_script)()
